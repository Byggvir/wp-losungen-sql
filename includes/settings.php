...
public function admin_init()
{
	...
	foreach ($HHL_DefValues as $key => $value) {
	       register_setting('hhl-losung', 'hhl_'.$key);
	}
    ...
    add_settings_section(
       	    'hhl_LosungenSection', 
       	    'HHL Losungen Einstellungen', 
       	    array(&$this, 'settings_section_losungen'), 
       	    'hhl_losungen'
       	);
	...
	foreach ($HHL_LosungenSection as $key => $value) {
          add_settings_field(
            'hhl_'.$key, 
            $HHL_DefLabels[$key], 
            array(&$this, 'settings_field_input_text'), 
...
public function settings_section_losungen() 
	{
